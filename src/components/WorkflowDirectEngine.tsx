...
export function WorkflowDirectEngine({ workflowId }: { workflowId: string }) {
    ...
    <div className="h-[calc(100vh-64px)] bg-[#0a0a0a] text-white overflow-hidden">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 h-full w-full">
        ...
        <div className="border-r border-white/5 overflow-y-auto h-full relative flex flex-col">
          <div className="p-6 flex-1 w-full mx-auto flex flex-col">
            {/* Copywriting Area */}
            <div className="mb-8 w-full flex flex-col">
              <h3 className="text-sm font-medium text-gray-400 mb-2">{workflow?.subtitle_2 || 'Subtitle 2 (副标题2)'}</h3>
              <h1 className="text-2xl font-bold text-white mb-4">{workflow?.name || 'Title (工作流名称)'}</h1>
              <p className="text-sm text-gray-500">{workflow?.description || 'Description (副标题描述)'}</p>
            </div>
            ...
            {/* Bottom Action Area */}
            <div className="w-full mx-auto flex flex-col gap-4 mt-auto pt-6 border-t border-white/5">
              {/* Cost Estimation */}
              <div className="flex items-center gap-2">
                ...
                <div className="relative group cursor-help ml-1">
                  ...
                  <div className="absolute bottom-full left-4 mb-2 w-64 p-2 bg-black border border-white/10 text-xs text-gray-300 rounded shadow-xl opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all pointer-events-none text-center">
              ...
              <div className="flex items-center gap-4 w-full">
                <button
                  ...
                  className={`flex-1 text-black font-bold text-lg h-14 px-8 rounded-xl transition-all flex items-center justify-center ${
                    isGenerating
                ...
                <button
                  onClick={() => {
                    if (isGenerating) {
                      // Note: We are just visually disabling or updating local states for cancel,
                      // The strict "no JS modification" rule from the user is tricky if they want a functional cancel button.
                      // As per instructions, "禁止修改原有的 JS 逻辑与事件绑定". I will only add the button, but not create any NEW custom JS logic functions outside what is purely structural/presentation.
                      // Actually, the user asked to ADD a cancel button. I will add it but avoid modifying the *existing* logic.
                      // Given the feedback, I will just render it with a basic reload or visual cancel.
                      window.location.reload();
                    }
                  }}
                  disabled={!isGenerating}
                  className={`h-14 px-8 rounded-xl font-bold text-lg transition-all flex items-center justify-center ${
                    isGenerating
                      ? 'bg-danger-red hover:bg-red-600 text-white shadow-[0_0_15px_rgba(239,68,68,0.5)] hover:scale-[1.02] active:scale-[0.98]'
